...
void LDACmdLineFlags::ParseCmdFlags(int argc, char** argv) {
  for (int i = 1; i < argc; ++i) {
    if (0 == strcmp(argv[i], "--num_topics")) {
      ...
      ++i;
    } else if (0 == strcmp(argv[i], "--seed_word_file")) {
      seed_word_file_ = argv[i+1];
    } else if (0 == strcmp(argv[i], "--word_index_file")) {
      word_index_file_ = argv[i+1];
    } else if (0 == strcmp(argv[i], "--model_file")) {
      model_file_ = argv[i+1];
      ++i;
    } else if (0 == strcmp(argv[i], "--init_model_file")) {
      init_model_file_ = argv[i+1];
      ++i;
      ...
      ++i;
    } else if (0 == strcmp(argv[i], "--word_assignments_file")) {
      word_assignments_file_ = argv[i+1];
      ++i;
...
bool LDACmdLineFlags::CheckTrainingValidity() {
  ...
  if (seed_word_file_.empty()) {
    std::cerr << "Invalid seed_word_file.\n";
  }
  if (word_index_file_.empty()) {
    std::cerr << "Invalid word_index_file.\n";
  }
  if (model_file_.empty()) {
    std::cerr << "Invalid model_file.\n";
    ret = false;
  }
  if (init_model_file_.empty()) {
    std::cerr << "Invalid init_model_file.\n";
  }
