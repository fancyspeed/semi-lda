...
class LDADocument {
 public:
  ...
  vector<int32> label_topics_;

  // Returns the document's word assignments.
  int word_assignments(map<int, int>& word_assign) {
    word_assign.clear();
    for (WordOccurrenceIterator iter(this);
         !iter.Done();
         iter.Next()) {
      word_assign[iter.Word()] = iter.Topic();
    }
    return word_assign.size();
  }
