...
class Sampler:
    ...
    def sample_doc(self, doc, update=False):
        for i, pair in enumerate(doc.word_list):
            ...
            if update:
                self.model.topic_word_count[word][topic] -= 1
            ...
            if update:
                self.model.topic_word_count[word][topic] = self.model.topic_word_count[word].get(topic, 0) + 1 
                self.model.topic_count[topic] += 1.
    ...
    def sample_word(self, word, doc):
        ...
        for topic in xrange(self.model.topic_num): 
            topic_word = self.model.topic_word_count[word].get(topic, 0) + self.model.beta
